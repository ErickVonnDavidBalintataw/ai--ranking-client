const URL = "https://ai-impact-ranking-server.onrender.com";
...
export const loginSubmit = async (loginCredentials) => {
  try {
    ...
    if (csdResponse.ok) {
      ...
      console.log(sdResponseData.token, 1, sdResponseData.user_id);
...
export const getInstruments = async () => {
  try {
    const response = await fetch(`${URL}/api/get/instrumentsbysdgandsection`);
    const data = response.json();
    return data;
  } catch (error) {
