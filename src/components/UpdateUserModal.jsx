...
const UpdateUserModal = ({ userId, setIsUpdateModalOpen, setReload }) => {
    ...
    const getUserDetails = async () => {
      try {
        const response = await fetch(
          `https://ai-impact-ranking-server.onrender.com/api/sd-office/${userId}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        const response = await fetch(
          `https://ai-impact-ranking-server.onrender.com/api/update/sd-office/${userId}`,
          {
            method: "PATCH", // Use PATCH or PUT for updates
            headers: {
              "Content-Type": "application/json",
            },
            body: JSON.stringify(formData), // Send the formData as request body
          }
        );
        ...
        <div className="p-4 space-y-4">
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4">
              ...
              {errors.campus_id && (
                <p className="text-red-500 text-sm">{errors.campus_id}</p>
              )}
            </div>
          </form>
        </div>
