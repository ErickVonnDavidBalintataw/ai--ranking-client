...
const UpdateRecordStatus = ({
    ...
    const fetchExistingAnswers = async () => {
      try {
        const response = await fetch(
          `https://ai-impact-ranking-server.onrender.com/api/get/answers/${recordId}`
        );
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchCampusData = async () => {
      try {
        const response = await fetch(
          `https://ai-impact-ranking-server.onrender.com/api/get/sd-office/${selectedSD}`
        );

        if (response.ok) {
          const data = await response.json();
          setCampusID(data[0].campus_id); // Set the campus state with fetched campus IDs
          console.log("Campus IDs fetched successfully: ", data[0].campus_id);
        } else {
  ...
  useEffect(() => {
        ...
        const updatedInstruments = await Promise.all(
          instrumentData.map(async (instrument) => {
            ...
            if (sectionsResponse.ok) {
              ...
              const sectionsWithQuestions = await fetchQuestionsForSections(
                sections
              );
              return {
                ...instrument,
    ...
    const fetchQuestionsForSections = async (sections) => {
      try {
          ...
          sections.map(async (section) => {
            try {
              ...
              const fetchEvidenceResponse = await fetch(
                `https://ai-impact-ranking-server.onrender.com/api/get/evidence/`,
                {
                  method: "POST",
                  headers: {
              ...
              const evidences = await fetchEvidenceResponse.json();
        ...
        setFormulas((prevFormula) => [...prevFormula, ...fetchedFormulas]);
  ...
  const handleInputChange = (e, question_id, campus_id) => {
    ...
    setAnswers((prevAnswers) =>
      prevAnswers.map((answer) =>
        answer.question_id === question_id && answer.campus_id === campus_id
  ...
  useEffect(() => {
    ...
    if (formulas && formulas.length > 0) {
      uniqueFormulas = formulas.reduce((acc, current) => {
        ...
        if (!acc.some((item) => item.formula_id === current.formula_id)) {
          acc.push(current);
        }
        return acc;
      }, []);
    ...
    if (answers && answers.length > 0) {
      const summedAnswers = answers.reduce((acc, item) => {
        ...
        const existingEntry = acc.find(
          (entry) => entry.question_id === questionId && entry.sub_id === subId
        );
  ...
  useEffect(() => {
    if (
      ...
      const uniqueFormulas = formulas.filter(
        (value, index, self) =>
          index === self.findIndex((t) => t.formula_id === value.formula_id)
      );
      ...
      const replaceFormulaValues = (formula, valueMap) => {
        return formula.replace(/([A-Z]\d+)/g, (match) => {
          return valueMap[match] !== undefined ? valueMap[match] : match;
        });
      };
      ...
      setTotal(updatedFormulasV.reduce((acc, curr) => acc + curr.score, 0));
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://ai-impact-ranking-server.onrender.com/api/update/status",
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
              ...
              <tbody className="bg-white divide-y divide-gray-200 text-sm">
                {instruments.map((instrument) => (
                  <React.Fragment key={instrument.instrument_id}>
                    <tr>
                      <td
                        colSpan={flattenedCampuses.length + 2}
                        className="px-6 py-4 font-semibold text-left bg-gray-100"
                      >
                        {instrument.sdg_subtitle}
                      </td>
                    </tr>
                    {instrument.section_contents.length > 0 ? (
                      instrument.section_contents.map((section) => (
                        <React.Fragment key={section.section_id}>
                          <tr>
                            <td className="px-6 py-4 whitespace-nowrap">
                              {section.section_content}
                            </td>
                            {flattenedCampuses.map((campus) => (
                              <td key={campus.id} className="border px-6 py-3">
                                {campus.name}
                              </td>
                            ))}
                          </tr>
                          ...
                          <tr>
                            ...
                            >
                              ...
                              {updatedFormulas
                                .filter(
                                  (formula) =>
                                    formula.section_id === section.section_id
                                )
                                .reduce(
                      ...
                      <tr>
                        <td
                          colSpan={flattenedCampuses.length + 1}
