...
const ScorePerCampusChart = ({ setScores, setTopCampus, selectedYear }) => {
  ...
  useEffect(() => {
    ...
    const fetchRecords = async () => {
      try {
        const response = await fetch(
          `https://ai-impact-ranking-server.onrender.com/api/get/records-values-by-sdg_id/${selectedSdg}/${selectedYear}`
        );
        const data = await response.json();
        console.log(data, "ASdasd");

        const section_response = await fetch(
          `https://ai-impact-ranking-server.onrender.com/api/get/sections/${selectedSdg}`
        );
        ...
        try {
          ...
          for (const section_id of uniqueSectionIds) {
            const response = await fetch(
              `https://ai-impact-ranking-server.onrender.com/api/get/formula/${section_id}`
            );
