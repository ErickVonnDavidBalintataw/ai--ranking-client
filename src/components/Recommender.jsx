  ...
  const fetchRecommendations = async (score, sdg_id) => {
    if (score) {
      ...
      const sdgsWithLowScores = score.records.filter(
        (record) => record.total_score < 1000 && record.sdg_id === sdg_id
      );
      ...
      for (const record of sdgsWithLowScores) {
        ...
        const prompt = `For campus: ${
          score.campus_name
        }, analyze the following SDG: ${record.sdg_no}. The total score is ${
          record.total_score
...
Return the answer in HTML as an ordered list. Add a line break after each </li> for readability.`;
        ...
        try {
          ...
          const cleanResponseContent = responseContent.replace(/\*/g, "");
  ...
  useEffect(() => {
    const fetchRecordsAndFormulas = async () => {
      try {
        ...
        const uniqueSectionContents = getUniqueSectionContents(recordsData);
  ...
  return (
    <Card className="w-[100%]">
      ...
      {recommendations && recommendations.length > 0 ? (
        ...
        <div className="text-justify">No recommendations available.</div>
      )}
    </Card>
  );
};
