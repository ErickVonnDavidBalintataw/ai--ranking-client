    ...
    setInstrumentData((prevData) => ({
      ...
      sections: [
        ...prevData.sections,
        {
          ...
          questions: [
            {
              questionId: generateQuestionId(prevData.sections.length, 0),
              questionText: "",
              questionType: "Number",
              suffix: "",
              options: [],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://ai-impact-ranking-server.onrender.com/api/add/instruments",
        {
          method: "POST",
          headers: {
        ...
        instrumentData.sections.map(async (section) => {
          ...
          await Promise.all(
            section.questions.map(async (questionData) => {
              const responseQuestion = await fetch(
                "https://ai-impact-ranking-server.onrender.com/api/add/questions",
                {
                  ...
                  body: JSON.stringify({
                    ...
                    section_id: responseSectionData.section_id,
                  }),
                }
              );
              ...
              const responseQuestionData = await responseQuestion.json();

              if (questionData.questionType === "Multiple Options") {
                await Promise.all(
                  questionData.options.map(async (option) => {
                    const responseOption = await fetch(
                      "https://ai-impact-ranking-server.onrender.com/api/add/options",
                      {
                        ...
                        body: JSON.stringify({
                          option,
                          question_id: responseQuestionData.question_id,
                        }),
                      }
                    );
          ...
          await Promise.all(
            section.formulas.map(async (formula) => {
              const responseFormula = await fetch(
                "https://ai-impact-ranking-server.onrender.com/api/add/formulas",
                {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
              ...
              {instrumentData.sections.map((section, sectionIndex) => (
                <div key={sectionIndex} className="section-group my-4">
                  ...
                  {section.questions.map((question, questionIndex) => (
                    <div key={questionIndex} className="question-group my-4">
                  ...
                  {section.formulas &&
                    section.formulas.map((formula, formulaIndex) => (
                      <div key={formulaIndex} className="formula-group my-2">
                          ...
                          <label className="form-control w-full">
                            ...
                            <div className="flex gap-2">
                              ...
                              <button
                                ...
                                className="bg-blue-600 text-white text-sm px-6 py-2 mt-2 h-fit"
                                onClick={() =>
                                  removeFormula(sectionIndex, formulaIndex)
                                }
                              >
                                <FontAwesomeIcon icon={faTrash} />
                              </button>
