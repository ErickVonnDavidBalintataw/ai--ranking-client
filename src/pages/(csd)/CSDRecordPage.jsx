  ...
  useEffect(() => {
    const fetchRecord = async () => {
      if (selectedYear && selectedSdg && selectedSdOffice) {
        ...
        try {
          const response = await fetch(
            `https://ai-impact-ranking-server.onrender.com/api/get/recordbysdoffice/${selectedYear}/${selectedSdg}/${selectedSdOffice}`
          );
          if (response.ok) {
            const data = await response.json();
            setRecord(data);
      ...
      <main className="h-full w-[80%] border overflow-auto">
        ...
        <div className="py-5 px-7">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="form__group">
              <label htmlFor="sdg" className="block text-gray-700 mb-2">
                Select SDG:
              </label>
              <select
                id="sdg"
            ...
            <div className="form__group">
              <label htmlFor="sdOffice" className="block text-gray-700 mb-2">
                Select SD Office:
              </label>
              <select
                id="sdOffice"
                value={selectedSdOffice}
                onChange={(e) => setSelectedSdOffice(e.target.value)}
                className="border p-2 w-full"
              >
                <option value="" disabled>
                  Select an SD Office
                ...
                {sdOffices.map((office) => (
                  <option key={office.user_id} value={office.user_id}>
                    {office.name} {/* Adjust this as per your data structure */}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="mt-4">
              {error ? (
                <div className="mt-4 p-4 border border-blue-500 bg-blue-100 text-blue-700 rounded-md flex items-center">
                  ...
                  <p className="text-sm">
                    <strong>Error:</strong> {error}. Please check the details
                    and try again.
                  </p>
                </div>
              ) : (
                <div className="mt-4 p-4 border border-blue-500 bg-blue-100 text-blue-700 rounded-md flex items-center">
                  ...
                  <p className="text-sm">
                    <strong>Notice:</strong> There are no existing records.
                  </p>
                </div>
              )}
            </div>
