  ...
  const filteredInstruments = instruments.filter((instrument) => {
    const matchesSearchQuery =
      ...
      instrument.number
        ...
        .toLowerCase()
        .includes(searchQuery.toLowerCase());
    const matchesSdg = selectedSdg === "" || instrument.sdg_id === selectedSdg;
  ...
  const handleStatus = async (e, status, instrument_id) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://ai-impact-ranking-server.onrender.com/api/update/instrument-status/${instrument_id}`,
        {
          method: "PATCH",
          headers: {
      ...
      <main className="h-full w-[80%] border overflow-auto">
        <div className="header py-5 px-7 flex justify-between items-center">
          ...
          {userRole.toString() === "0" && ( // Check if the user role is '0'
            ...
            >
              <FontAwesomeIcon icon={faSquarePlus} /> Add New Instrument
            </Link>
          )}
        </div>
        <hr />
                    ...
                    <td className="px-6 py-3 whitespace-nowrap text-sm font-medium">
                      {userRole.toString() === "1" ? ( // Check if the user role is '1'
                        ...
                        <>
                          <button
                            onClick={(e) =>
                              handleStatus(e, row.status, row.instrument_id)
                            }
