...
const ViewInstrumentPage = () => {
  ...
  useEffect(() => {
    const fetchInstrument = async () => {
      const response = await fetch(
        `https://ai-impact-ranking-server.onrender.com/api/get/instruments/${instrument_id}`
      );
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      const response = await fetch(
        `https://ai-impact-ranking-server.onrender.com/api/get/questions/${instrument?.section_id}`
      );
      const data = await response.json();
      setQuestions(data);
    };
    ...
    const fetchFormula = async () => {
      const response = await fetch(
        `https://ai-impact-ranking-server.onrender.com/api/get/formula/${instrument?.section_id}`
      );
      const data = await response.json();
      setFormula(data[0]);
      console.log(data);
    ...
    <section className="h-screen flex">
      ...
      <main className="h-full w-[80%] border overflow-auto">
        <div className="header py-5 px-7 flex justify-between items-center">
          <h1 className="text-2xl text-gray-900">View Instrument Details</h1>
          <Link
            to="/csd/instruments"
        ...
        <div className="bg-white shadow rounded-lg p-6">
          <table className="w-full table-auto border-collapse">
            <tbody>
              <tr>
                <th className="bg-red-500 text-white p-4 text-left" colSpan={2}>
              ...
              <tr className="border">
                ...
                <td className="px-6 py-3 text-left text-xs font-medium text-black uppercase tracking-wider">
                  SDG {instrument?.number + ": " + instrument?.title}
                </td>
              </tr>
              ...
              <tr className="border bg-red-100">
                ...
                <td className="px-6 py-3 text-left text-xs font-medium text-black uppercase tracking-wider">
                  {questions.map((question, index) => (
                    <p key={index} className="text-wrap mb-2 text-gray-700">
              ...
              <tr className="border-b bg-gray-50">
                ...
                <td className="px-6 py-3 text-left text-xs font-medium text-black uppercase tracking-wider">
                  {formula?.formula ? formula?.formula : "No formula available"}
                </td>
              </tr>
            </tbody>
          </table>
