...
const AnnualReportPage = () => {
  ...
  const fetchReports = async () => {
    try {
      const response = await fetch(
        "https://ai-impact-ranking-server.onrender.com/api/get/annual-reports"
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://ai-impact-ranking-server.onrender.com/api/upload-annual-report",
        {
          method: "POST",
          body: formData,
        }
      ...
      <main className="h-full w-[80%] border overflow-auto">
        ...
        <div className="py-4 px-7">
          ...
          >
            ...
            {Array.from({ length: 5 }, (_, i) => 2024 - i).map((yearOption) => (
          ...
          {filteredReports.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredReports.map((report) => (
                <div key={report.annual_id} className="flex justify-center">
                  <button
                    onClick={() =>
                      window.open(
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center">
            <div className="bg-white p-8 rounded-lg w-1/3">
              <h2 className="text-xl mb-4">Upload Annual Report</h2>
