...
const SDRecordPage = () => {
  ...
  useEffect(() => {
    const runValidation = async () => {
      try {
        ...
        if (!isVerified || localStorage.getItem("role").toString() !== "1") {
  ...
  useEffect(() => {
    const fetchInstrumentsBySdg = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch instruments for SDG ${selectedSdg}`);
        }
  ...
  useEffect(() => {
    ...
    instruments.forEach((instrument) => {
      instrument.sections.forEach((section) => {
        section.formula.forEach((formulaObj) => {
          ...
          const variables = section.questions.reduce((acc, question) => {
          ...
          const substitutedFormula = substituteVariables(formula, variables);
    ...
    setScores((prevScores) => {
      ...
      setSum(
        updatedScoresArray.reduce((sum, score) => sum + (score.result || 0), 0)
      );
                    ...
                    <div className="flex justify-end">
                      <h1 className="mt-4">
                        ...
                        <span className="text-green-500">
                          {scores
                            .filter(
                              (score) => score.section_id === section.section_id
                            )
                            .map((score, index) => (
                              <div key={index}>{score.result}</div>
                            ))}
                        </span>
                      </h1>
                    </div>
